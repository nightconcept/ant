...
- Area: The benchmark threshold gate cannot compare across machines
  - Issue: `check_thresholds` in `bench/bench.py` diffs a run against `docs/repo/bench-baseline.json` on absolute milliseconds, and that baseline is recorded on whatever machine ran `just bench-update-baseline`. On a GitHub runner every benchmark then reads 15-47% slower than a laptop-recorded baseline with no code change, so the gate fails unconditionally. Gating on the ratio to the reference runtime instead does not fix it: measured between two runs of *identical* Ant code on one machine with the same node build (v26.5.0), Ant's own fastest-sample time drifts a median 1.6% / worst 5.4%, while node drifts worst 15.0% on `best` and 28.5% on `work` — subtracting node's ~18ms startup floor from its small numbers amplifies its noise, so the reference is noisier than the subject and `Class & Megamorphic Dispatch` alone moved its ratio +42.5%. A ratio gate wide enough not to flake (~30%) would only catch gross regressions.
  - Impact: `main-ci.yml` and `upstream-ci.yml` carry no performance gate; the step is commented out in both. Performance regressions are caught only by running `just bench-fast-diff` locally against a locally-recorded baseline, which is a valid Ant-vs-Ant comparison but is not enforced. The gate was previously inert — before the two-tier bench work the flags were parsed by nothing and the job could not fail — so this removes a gate that never actually ran, rather than one that was working.
  - Proposed fix: Record the baseline on the same runner class CI uses and gate Ant-vs-Ant absolute time there, which fits inside Ant's measured 5.4% worst-case drift and so works at the existing 6% threshold. That needs a way to promote a CI-produced manifest to the baseline, and a decision about what `just bench-fast-diff` compares against locally once the checked-in baseline describes CI hardware — most likely a second, local baseline, with `bench-history.jsonl` carrying local trends. Until then do not re-enable the step: it will fail on every push.
  - Status: backlog
