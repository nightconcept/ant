...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Run all local Tier 3 spec tests")
    parser.add_argument("-f", "--filter", type=str, help="Filter tests by substring")
    args = parser.parse_args()

    ant_bin = find_ant_binary()
    tracker = SummaryTracker("Tier 3 - Full Conformance (Test262 / WPT / Frameworks)")
    ...
    if args.smoke or not args.all:
        ...
        print("Running Tier 3 spec files...")
        spec_dir = REPO_ROOT / "examples" / "spec"
        spec_files = sorted([f for f in spec_dir.glob("*.js") if f.name in ("observable.js", "atomics.js", "explicit_resource_management.js")])
