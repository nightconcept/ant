...
function evalA(i: number, j: number): number {
    return 1.0 / (((i + j) * (i + j + 1)) / 2 + i + 1);
}

function evalAtimesFortran(v: Float64Array, Atv: Float64Array, n: number): void {
    for (let i = 0; i < n; i++) {
        let sum = 0.0;
        for (let j = 0; j < n; j++) {
            sum += evalA(i, j) * v[j];
...
function evalAtAtimesFortran(v: Float64Array, AtAv: Float64Array, u: Float64Array, n: number): void {
